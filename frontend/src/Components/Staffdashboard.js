    ...
    //search button
        ...
        const searchSubjects = async () => {
            try {
                const response = await axios.get(`/subjects/getSubject?searchTerm=${searchTerm}`);
                if (response.status === 200) {
        ...
        const handleSubmitStudent = async (e) => {
            e.preventDefault();
            try {
                const response = await axios.post('/staff/createTuitionList', studentFormData);
                if (response.status === 201) {
        ...
        return (
            ...
            <div>
                ...
                <div className="search-container">
                    <input className="search-bar" placeholder="Search" value={searchTerm} onChange={handleSearchInputChange} />
                ...
                <tbody>
                    {(searchResults.length > 0 ? searchResults : subjects).map((subject, index) => (
                        <tr key={index}>
                            ...
                            <td>
                                <button onClick={() => handleDeleteSubject(subject.Subject_Code)} className="delete-button">Delete</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
                    ...
                    <div className="smodal">
                        <div className="smodal-content">
                            ...
                            <form onSubmit={handleSubmitStudent}>
                                ...
                                <input type="checkbox" name="Prelim_Status" checked={studentFormData.Prelim_Status} onChange={() => setStudentFormData({ ...studentFormData, Prelim_Status: !studentFormData.Prelim_Status })} />
                                <label>Midterm Status:</label>
                                <input type="checkbox" name="Midterm_Status" checked={studentFormData.Midterm_Status} onChange={() => setStudentFormData({ ...studentFormData, Midterm_Status: !studentFormData.Midterm_Status })} />
                                <label>SemiFinal Status:</label>
                                <input type="checkbox" name="SemiFinal_Status" checked={studentFormData.SemiFinal_Status} onChange={() => setStudentFormData({ ...studentFormData, SemiFinal_Status: !studentFormData.SemiFinal_Status })} />
                                <label>Final Status:</label>
                                <input type="checkbox" name="Final_Status" checked={studentFormData.Final_Status} onChange={() => setStudentFormData({ ...studentFormData, Final_Status: !studentFormData.Final_Status })} />
                                <button type="submit">Create</button>
                            </form>
                        </div>
                    </div>
